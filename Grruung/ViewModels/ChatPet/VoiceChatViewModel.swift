...
class VoiceChatViewModel: ObservableObject {
    ...
    @Published var voiceType: SpeechService.VoiceType = .human      // 음성 타입
        ...
        vertexService.generatePetResponse(prompt: contextualPrompt) { [weak self] response, error in
            ...
            DispatchQueue.main.async {
                ...
                if let response = response, !response.isEmpty {
                    ...
                    // 중요한 대화 내용 분석 및 저장
                    self.analyzeAndStoreImportantContent(userInput: userSpeech, response: filteredResponse)
                } else {
                    // 응답 생성 실패 시 기본 메시지
    ...
    func endCurrentSession(completion: @escaping (Error?) -> Void) {
        guard let sessionID = currentSessionID else {
            completion(NSError(domain: "VoiceChatViewModel", code: 404, userInfo: [NSLocalizedDescriptionKey: "활성 세션이 없습니다."]))
            return
        }
