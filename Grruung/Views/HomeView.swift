...
// TODO: 2. 각 시기(운석(알): 50, 유아기: 100, 소아기: 150, 청년기: 200, 성년기: 300, 노년기: 500 별로 레벨업하는 경험치 요구량 고정
// TODO: 3. 운석(알)때는 튜토리얼 개념으로 경험치 빨리 획득하게 해서 최대한 빨리 유아기로 갈 수 있게 설정 (기본 획득량이 3이면 운석(알)에서만 5배 빨리 획득 이런식)
// TODO: 4. 상태바 프로그레스 스텟 80이상 파란색 / 21~79 녹색 / 20이하 빨간색 으로 나오게 하기
// TODO: 5. 보이는 스텟 (포만감, 운동량)은 일정시간 마다 -1씩 깎이고 / 히든 스텟 (건강, 청결)도 보이는 스텟보다는 긴 일정 시간이후로 -되고 / 애정도는 매일 06시 기준 활동 한번도 안했으면 - 되게
// TODO: 6. 활동버튼은 액션마다 추가 해놓고 랜덤으로 나오게 하기. / 22:00 ~ 06:00 은 잠자기 무조건 나오게 -> 이건 추후 마이페이지 설정에서 변경하거나 워치 연결 시 수면 시간에 맞춰서 나오게 변경
// TODO: 7. 동산 버튼이 누르면 현재 키우던 펫을 캐릭터뷰에 마지막 상태, 스텟, 대화내용, 들어준이야기 내용들 저장 후 홈뷰는 빈상태 -> 펫추가(처음부터 새로 키울 수 있게) 뷰로 변경
// TODO: 8. 사이드 버튼 중 잠겨 있는 버튼 - 유아기~노년기 각 성장 시기마다 특정 조건을 달성하면 히든 활동(또는 스토리) 등장. → 지용님 확인하고 답변좀
...
//
//
// 0~8 작업 완료 후 → 테스트 시에는 모든 수치 증가 5~10배 적용
