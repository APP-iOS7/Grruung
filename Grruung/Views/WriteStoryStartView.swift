...
struct WriteStoryStartView: View {
    ...
    init(characterUUID: String, userId: String) {
        self.characterUUID = characterUUID
        _viewModel = StateObject(wrappedValue: WriteStoryViewModel())
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                Spacer()
                Button {
                    if writingCountVM.userWritingCount.totalAvailableCount > 0 {
                        // 글쓰기 횟수가 남아있으면 이동
                    ...
                    // 글쓰기 횟수의 남은 횟수에 따라 버튼 색상 변경 글쓰기 가능: 파랑, 불가능: 회색
                        .background(writingCountVM.userWritingCount.totalAvailableCount > 0 ? Color.blue : Color.gray)
                        .clipShape(Circle())
                        .shadow(radius: 4)
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                Text("남은 글쓰기 횟수: \(writingCountVM.userWritingCount.totalAvailableCount)")
                    .font(.subheadline)
                    .foregroundColor(.gray)
                    .padding(.top, 8)
            }
